...
def aStar(grid):
    ...
    while queue:
        ...
        rows = [red_car.start_y-2, red_car.start_y-1, red_car.start_y+1, red_car.start_y+2]
                ...
                if grid.check_move_car(car_n, 1):
                    ...
                    if new_grid.grid not in done_states:
                        ...
                        for i in range(red_car.start_x + red_car.length, 6):
                            car_value = new_grid.retrieve_value(i, red_car.start_y)
                            if car_value != 0:
                                cost += 1
                                for x in rows:
                                    if car_value != new_grid.retrieve_value(i, x):
                                        cost += 1
